#id shorttrack_smooth_back_and_forth.py 2024-12-17 oh
# MovingCap flatTRACK/shortTRACK application demo
# smooth&quiet back and forth motion for new RABBIT position control 
# precondition: 
# - firmware v53.02.00.43 or higher 
# - softlimit object 607Dh.1h and 607D.2h set
import sys
import mcdrive as mc

PARAMETER_ADJUSTMENTS = """
050.6067h.00h,unsigned32,50 # Position window
050.6068h.00h,unsigned16,100 # Position window time
050.6081h.00h,unsigned32,17000 # Profile_velocity
050.6083h.00h,unsigned32,1000 # Profile acceleration
"""

def main():
    """
    flatTRACK/shortTRACK application demo

    - use the parameters from PARAMETER_ADJUSTMENTS
    - check softlimit 
    - wait 5 seconds
    - start motion between softlimit min and max
    """

    WriteObjectsFromKickdriveList(PARAMETER_ADJUSTMENTS)
    ...
    if ((softLimitMin == 0 and softLimitMax == 0) or softLimitMax <= (softLimitMin + 1000)):
        print("flatTRACK abort. Check softlimit objects 607Dh.1h and 607D.2h!") 
        return
    
    # initial wait, don't surprise me with immediate movement
    sys.wait(5000)

    # enable operaton and go
    mc.EnableDrive()
    while(1):
...
def ChkReady(): 
    ...
    while (mc.ChkReady() == 0):
        sys.wait(1)
    # only continue if no error. restart attempt after approx 10 seconds
    restartTimer = 0
    while ((restartTimer < 10000) and (mc.ChkError() > 0)):
        sys.wait(1)
        restartTimer = restartTimer + 1
    if (restartTimer >= 10000): 
